...
if [ $(id -u) == 0 ] ; then
     ...
     # Only attempt to change the elrond username if it exists
    if id elrond &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER elrond
    fi
    ...
    if [[ "$NB_USER" != "elrond" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/elrond "/home/$NB_USER"
        fi
        # if workdir is in /home/elrond, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/elrond/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u elrond)" && "$NB_GID" == "$(id -g elrond)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^elrond:/nayvoj:/" > /tmp/passwd
                echo "elrond:x:$(id -u):$(id -g):,,,:/home/elrond:/bin/bash" >> /tmp/passwd
